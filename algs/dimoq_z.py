...
class DIMOQZ:
    ...
    This algorithm is exactly the same as DIMOQ, but uses next state sets rather than state-action-next state sets.
    This is equivalent but ensures faster learning. In fact, this is basically value iteration on a learned model.
    """
    ...
    def get_q_dists(self, state, action):
        ...
        for next_state in range(self.num_states):
            ...
            if prob > 0:
                ...
                for nd_dist in self.return_dists[next_state]:
    ...
    def train(self, num_episodes=3000, warmup_time=1000, learn_model=False, log_every=100, action_eval='linear'):
        ...
        for episode in range(num_episodes):
            ...
            while not (terminated or truncated):
                ...
                self.return_dists[next_state] = get_best(new_nd, max_dists=self.max_dists, rng=self.rng)
