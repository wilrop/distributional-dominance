...
def c_prune(candidates):
    ...
    while p_candidates:
        ...
        if weight is None:  # If there is none, discard the distribution.
            ...
            new_dist = max(p_candidates, key=lambda x: np.dot(x.get_expected_value(), weight))
...
def pareto_dominates(a, b):
    ...
    """
    ev_a = a.get_expected_value()
    ev_b = b.get_expected_value()
...
def find_weight(dist, candidates):
    ...
    """
    vector = dist.get_expected_value()
    candidates = [dist.get_expected_value() for dist in candidates]
